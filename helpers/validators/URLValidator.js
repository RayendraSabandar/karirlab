function linkedInValidator(linkedin_url) {
    ...
    if(!linkedInRegex.test(linkedin_url)) {
        throw new Error(`Must enter a valid LinkedIn URL; Remove the key if you didn't want to put a valid URL`);
    } 
}
...
function portfolioValidator(portfolio_url){
    ...
    if(!urlRegex.test(portfolio_url)) {
        throw new Error(`Must enter a valid Portfolio URL; Remove the key if you didn't want to put a valid URL`)
    }
}
